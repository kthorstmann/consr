...
freq_overview(item.dirty, range = c(1, 5))

```

Using `freq_overview` will give us an overview over each answer on the item. Using the `range` argument will tell the function, what range the liker scale actually has and thereby, which of the values are valid.
...
data["var_j"] <- data["var_j"] / 2.3 # get one non-numeric
...
If you every have trouble using this argument, recode your data properly before computing the consensus and set `range = NULL`, which is the default. It will then be guessed from your data frame for each item.
